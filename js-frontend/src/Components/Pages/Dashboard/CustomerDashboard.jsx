...
import { getCustomerJobs, deleteJob } from '../../api/client';
...
const CustomerDashboard = () => {
  ...
  const { isLoading: isLoadingPosting } = useQuery(['customer-jobs'], () => getCustomerJobs(), {
    enabled: userType === 'customer', // only fetch when customer is logged in
    onSuccess: ({ data }) => setPostings(data),
    onError: (err) => toast.error(err.response?.data.message || err.message),
  });

  const { mutate } = useMutation((jobId) => deleteJob(jobId), {
    onSuccess: () => client.invalidateQueries('customer-jobs'),
    onError: (err) => toast.error(err.response?.data.message || err.message),
  });
        ...
        <Grid item xs={12} sm={12} md={8}>
          <div className="card shadow border-radius mb-4">
            <div className="card-head d-flex justify-content-between border-bottom p-3">
              ...
              <p className="mb-0 txt-primary cursor-pointer" onClick={toggleShowPostings}>
            ...
            <div className="card-body p-3">
              <Grid container spacing={3}>
                <Switch>
                  ...
                  <Case condition={postings.length === 0}>
                    <Typography variant="body2" textAlign="center">
                      No jobs yet
                    </Typography>
                  </Case>
                  <Case condition={postings.length > 0}>
                    {postings.slice(0, showAllPostings ? undefined : 3).map((item, idx) => (
