...
const CustomerNotifications = () => {
  ...
  useQuery(['customer-notifications-count'], () => getNotificationsCount(), {
  ...
  const { mutate: markAsReadNotification } = useMutation((id) => readNotification(id), {
    onSuccess: () => client.invalidateQueries(['customer-notifications']),
    onError: (err) => toast.error(err.response?.data.message || err?.message),
  });

  const { mutate: deleteCustomerNotification } = useMutation((id) => deleteNotification(id), {
    onSuccess: () =>
      client.invalidateQueries(['customer-notifications', 'customer-notifications-count']),
    onError: (err) => toast.error(err.response?.data.message || err?.message),
  });
                      ...
                      return (
                        <TableRow hover key={row._id}>
                          ...
                          <TableCell>
                            ...
                            <IconButton
                              color="error"
                              onClick={() => deleteCustomerNotification(row._id)}
                            >
                              <Delete />
                            </IconButton>
                          </TableCell>
